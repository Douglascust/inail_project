$(document).ready(function () {
	...
	function showEvents(response){
		...
		for(let i = 0; i < response.results.length; i++){
			events += '<tr>\
						<td class="idCliente">'+response.results[i].idCliente+'</td>\
						<td class="nome">'+response.results[i].nome+'</td>\
                        <td class="email">'+response.results[i].email+'</td>\
                        <td class="cpf">'+response.results[i].cpf+'</td>\
                        <td class="senha">'+response.results[i].senha+'</td>\
                        <td>\
	...
	$(document).on("click", "#btneditar", function(){
		...
		var cpf = linha.find(".cpf").text();
        var email = linha.find(".email").text();
        var senha = linha.find(".senha").text();

		$("#editarForm #nome").val(nome);
		$("#editarForm #cpf").val(cpf);
		$("#editarForm #email").val(email);
		$("#editarForm #senha").val(senha);
		
		
		
		$("#modaledituser").modal("show");
	
	    // Configurar o botão de confirmação do modal de exclusão
	    $("#editarForm").off("submit").on("submit", function(event){
			event.preventDefault();
			var acao = "atualizar";
			var idCliente = linha.find(".idCliente ").text();
			var nome = $("#editarForm #nome").val();
			var cpf = $("#editarForm #cpf").val();
			var email = $("#editarForm #email").val();
			var senha = $("#editarForm #senha").val();
			...
			if(validarCpf(cpf)){
	    		...
	    		if(validarEmail(email)){
					...
					let newBtn = '<button class="btn btn-primary" type="button" disabled>\
						  <span class="spinner-border spinner-border-sm" aria-hidden="true"></span>\
						  <span class="visually-hidden" role="status">Loading...</span>\
					</button>';
					let oldBtn = '<input type="submit" name="action" id="action" class="btn btn-primary" value="Editar">';
					$("#editBtn").html(newBtn);
					setTimeout(function(){ 
						$.ajax({
							url:"ctrlcliente",
	...
	$(document).on("click", "#btnexcluir", function(){
	    ...
	    // Abra o modal de exclusão manualmente
	    $("#modaldeluser").modal("show");
	
	    // Configurar o botão de confirmação do modal de exclusão
	    $("#excluirForm").off("submit").on("submit", function(event){
	        event.preventDefault();
	        var acao = "excluir";
			var idCliente = linha.find(".idCliente ").text();
			var nome = linha.find(".nome").text();
	...
	$("#cadastrarmaniForm").submit(function(event){
		event.preventDefault();
		var acao = "inserir";
		var idCliente = 0;
		...
		if(validarCpf(cpf)){
    		...
    		if(validarEmail(email)){
				...
				let oldBtn = '<input type="submit" name="action" id="action" class="btn btn-primary" value="Adicionar">';
