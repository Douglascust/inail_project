...
class Sanitize {
    ...
    public function clearRequestHttp() {
        ...
        if ($this->getCleanRequestVars()) {
            ...
            $_GET = array_map('filter_var', $_GET, array_fill(0, count($_GET), 513 ));
            $_POST = array_map('filter_var', $_POST, array_fill(0, count($_POST), 513));
            $_REQUEST = array_map('filter_var', $_REQUEST, array_fill(0, count($_REQUEST), 513));
            $this->input_vars = array_map('filter_var', $this->input_vars, array_fill(0, count($this->input_vars), 513));
    ...
    public function toClean($data) {
        ...
        } elseif (strtotime($data) !== false &&  strlen($data) > 10) {
